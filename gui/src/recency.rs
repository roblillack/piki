//! Tracks when each note was last opened.
//!
//! The note picker lists notes most-recently-opened first and lets a double
//! `Cmd-O`/`Ctrl-O` jump straight back to the previous note. Both need to know
//! *when* notes were last opened, which the in-session navigation [`History`]
...
//! [`crate::window_state::recent_notes_file`]) so different wikis keep separate
//! histories.
//!
...
pub struct RecentNotes {
    /// Note name -> last-opened time in milliseconds since the Unix epoch.
    #[serde(default)]
...
impl RecentNotes {
    ...
    /// Record that `note` was opened just now.
    pub fn mark_opened(&mut self, note: &str) {
        self.opened.insert(note.to_string(), now_millis());
    }

    /// The last-opened time (ms since epoch) for `note`, if it has been opened.
    pub fn last_opened(&self, note: &str) -> Option<i64> {
        self.opened.get(note).copied()
    }
...
mod tests {
    ...
    fn roundtrips_names_with_slashes() {
        ...
        let back: RecentNotes = toml::from_str(&toml).unwrap();
